...
class _ReasonsSettingsPageState extends State<ReasonsSettingsPage> {
  ...
  void _addReason(BuildContext context, List<String> currentReasons) {
    ...
    if (reason.isNotEmpty && !currentReasons.map((e) => e.toLowerCase()).contains(reason.toLowerCase())) {
      final updated = List<String>.from(currentReasons)..add(reason);
      context.read<SettingsBloc>().add(UpdateMissedReasons(missedReasons: updated));
      _reasonController.clear();
    }
  }

  void _removeReason(BuildContext context, List<String> currentReasons, String reason) {
